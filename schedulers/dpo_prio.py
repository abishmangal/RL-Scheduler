...
======================
CPU scheduler using DPO-trained policy for updated environment (9 features)
Includes time quantum, queue position, wait time tracking, and time since last run.
"""
...
class DPOPriority(Scheduler):
    ...
    For updated environment: 9 features, time quantum, dictionary tracking.
    ...
    def __init__(self, data, **kwargs):
        # Initialize parent Scheduler with data (sets pids, arrivals, instr_count)
        super().__init__(data)
    ...
    def _get_observation(self, data_pointer: int, processes: list, current_time: int) -> np.ndarray:
        """
        Build observation for updated environment (9 features).
